...
export default function PurchaseOrderMailModal({ orderNo, subjectHint, fetchPdfBlob, onClose }: Props) {
  ...
  const requestDraft = async () => {
    ...
    try {
      const r = await api.post<{ subject: string; body: string }>('/emails/purchase_order_draft', {
        order_no: orderNo,
        subject: subjectHint,
      })
